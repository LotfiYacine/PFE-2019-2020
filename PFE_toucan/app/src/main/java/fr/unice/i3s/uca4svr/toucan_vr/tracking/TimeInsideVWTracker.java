...
public class TimeInsideVWTracker {
    ...
    /**
     * Initialize a {@link BandwidthConsumedTracker}, that will record the time
     * inside during playback to a file name logFilePrefix_TimeInsideVW_date.csv.
     *
     * @param logFilePrefix The prefix for the log file name
     */
    public TimeInsideVWTracker(String logFilePrefix) {
        ...
        logger = LoggerFactory.getLogger("fr.unice.i3s.uca4svr.tracking.TimeInsideVWTracker"
                + loggerNextID++);
    ...
    private String createLogFileName(String logFilePrefix) {
        ...
        Date date = new Date();
        return String.format("%s_TimeInsideVW_%s.csv", logFilePrefix, dateFormat.format(date));
    }

    public void track(long time, long timeInsideVirtualWall, long currentVideoTime, boolean userLookingOutOfWallsLimits,long timeOutSideVirtualWall) {
        String string = String.format(Locale.ENGLISH, "%d,%d,%d,%b,%d",
                time, timeInsideVirtualWall, currentVideoTime, userLookingOutOfWallsLimits,timeOutSideVirtualWall);
        logger.error(string);
    }
}
