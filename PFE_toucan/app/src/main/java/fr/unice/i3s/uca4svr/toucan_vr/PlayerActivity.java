...
import fr.unice.i3s.uca4svr.toucan_vr.tracking.StartTimeTracker;
import fr.unice.i3s.uca4svr.toucan_vr.tracking.TapTracker;
...
public class PlayerActivity extends GVRActivity implements RequestPermissionResultListener, CheckConnectionResponse {
    ...
    private StartTimeTracker startTimeTracker;
    //bool for VW strategies
    boolean tap;
    boolean insistence;
    ...
    private void parseIntent() {
        ...
        //get VW strategies
        tap = intent.getBooleanExtra("tap", false);
        insistence = intent.getBooleanExtra("insistence", false);
        ...
        //put VW strategies
        dynamicEditingHolder.tap = this.tap;
        dynamicEditingHolder.insistence = this.insistence;
        if(tap){
            logPrefix=logPrefix+"_tap";
        }else if(insistence){
            logPrefix=logPrefix+"_insistence";
        }else{
            logPrefix=logPrefix+"_ref";
        }
        dynamicEditingHolder.logPrefix=logPrefix;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        synchronized (this) {
            if ( this.statusCode == Status.PLAYING && dynamicEditingHolder.tap && event.getActionMasked() == MotionEvent.ACTION_UP &&
                    event.getEventTime() - event.getDownTime() < 1000) {
                if (dynamicEditingHolder.isDynamicEdited()) {
                    if ( dynamicEditingHolder.getCurrentOperation() != null) {
                        if (dynamicEditingHolder.getCurrentOperation().isReady(player.getCurrentPosition())) {
                            dynamicEditingHolder.getCurrentOperation().deactivate(((Minimal360Video) getMain()).getVideoHolder(), videoSceneObjectPlayer);
                        }
                    }
                }
                final Clock clock = new SystemClock();
                tapTracker.track(clock.elapsedRealtime(), player.getCurrentPosition(), Angles.getCurrentYAngle(((Minimal360Video) getMain()).getVideoHolder().getGVRContext()));
            } else if (event.getActionMasked() == MotionEvent.ACTION_UP &&
                    event.getEventTime() - event.getDownTime() > 2000) {
                switch (statusCode) {
                    case NO_INTENT:
                        break;
                    case NO_PERMISSION:
                ...
                final Clock clock = new SystemClock();
                Date now = new Date();
                SimpleDateFormat sdfStopTime = new SimpleDateFormat("HH:mm:ss:SSS");
                String formattedTime = sdfStopTime.format(now);
                startTimeTracker.track(formattedTime, clock.elapsedRealtime());
            }
        }
        return true;
    }
        ...
        synchronized (this) {
            ...
            if (statusCode != Status.NO_INTERNET && statusCode != Status.NO_INTENT) {
                if (result == PackageManager.PERMISSION_GRANTED) {
                    tapTracker = new TapTracker(logPrefix);
                    startTimeTracker = new StartTimeTracker(logPrefix);
