...
import fr.unice.i3s.uca4svr.toucan_vr.tracking.writers.InterruptedVirtualWallLogWriter;
...
public class DynamicOperationsTracker {
    ...
    private InterruptedVirtualWallLogWriter interruptedVirtualWallLogWriter;
    ...
    public DynamicOperationsTracker(String logFilePrefix) {
        ...
        interruptedVirtualWallLogWriter = new InterruptedVirtualWallLogWriter(logFilePrefix);
    }
...
//    }
    ...
    /**
     * Outputs a track record to the InterruptedVirtualWallLogWriter log file.
     * Each entry in the csv will have:
     * - the timestamp of the virtual wall
     * - the timestamp at which the virtual wall is activated
     * - the duration of the virtual wall
     * - the position in the video where the virtual wall should end
     * - the position in the video where the virtual wall was interrupted
     *
     * @param expectedTS        the timestamp of the virtual wall in the XML file
     * @param actualTS          the timestamp at which the virtual wall is activated
     * @param expectedDuration  the duration of the virtual wall
     * @param endVideoTime      time in the video where the virtual wall is supposed to end
     * @param realEndVideoTime  time in the video where the virtual wall was interrupted
     */
    public void InterruptedVirtualWallLogWriter(long time,long expectedTS, long actualTS, long expectedDuration, long endVideoTime,
                                                long realEndVideoTime,float currentAngle) {
        interruptedVirtualWallLogWriter.writeLine(String.format(Locale.ENGLISH, "%d,%d,%d,%d,%d,%d,%f",
                time,expectedTS, actualTS, expectedDuration, endVideoTime, realEndVideoTime,currentAngle));
    }
}
