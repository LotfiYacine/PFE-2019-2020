...
public class HeadSpeedTracker {
    ...
    /**
     * Initialize a {@link HeadSpeedTracker}, that will record the head speed
     * during virtual wall to a file name logFilePrefix_s_HeadSpeed__date.csv.
     *
     * @param logFilePrefix The prefix for the log file name
     */
    public HeadSpeedTracker(String logFilePrefix) {
        ...
        logger = LoggerFactory.getLogger("fr.unice.i3s.uca4svr.tracking.HeadSpeedTracker"
                + loggerNextID++);
    ...
    private String createLogFileName(String logFilePrefix) {
        ...
        Date date = new Date();
        return String.format("%s_HeadSpeed_%s.csv", logFilePrefix, dateFormat.format(date));
    }

    public void track(float speed, long currentVideoTime) {
        String string = String.format(Locale.ENGLISH, "%f,%d",
                speed, currentVideoTime);
        logger.error(string);
    }
}
