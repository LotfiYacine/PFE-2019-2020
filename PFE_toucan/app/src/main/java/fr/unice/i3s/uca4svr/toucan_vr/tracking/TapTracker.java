...
public class TapTracker {
    ...
    /**
     * Initialize a {@link TapTracker}, that will record the tap
     * during video to a file name logFilePrefix_s_Tap__date.csv.
     *
     * @param logFilePrefix The prefix for the log file name
     */
    public TapTracker(String logFilePrefix) {
        ...
        logger = LoggerFactory.getLogger("fr.unice.i3s.uca4svr.tracking.TapTracker"
                + loggerNextID++);
    ...
    private String createLogFileName(String logFilePrefix) {
        ...
        Date date = new Date();
        return String.format("%s_Tap_%s.csv", logFilePrefix, dateFormat.format(date));
    }

    public void track(long time,long currentVideoTime,float currentAngle) {
        String string = String.format(Locale.ENGLISH, "%d,%d,%f",
                 time,currentVideoTime,currentAngle);
        logger.error(string);
    }
}
